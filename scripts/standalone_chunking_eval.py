#!/usr/bin/env python3
"""
Standalone script to evaluate chunking methods without project dependencies.
"""
...
class SemanticChunkingService:
    ...
    def _split_into_sentences(self, text: str) -> List[str]:
        ...
        # Simple sentence splitting - in practice, you might want to use nltk or spacy
    ...
    def chunk_text(self, text: str, chunk_size: int, overlap: int) -> List[Tuple[str, int, int]]:
        ...
        current_chunk_start = text.find(sentences[0])
        
        for i in range(1, len(sentences)):
            # Calculate similarity between current sentence and previous chunk
            prev_chunk_embedding = np.mean(
                [sentence_embeddings[j] for j in range(len(current_chunk_sentences))], 
                axis=0
            ).reshape(1, -1)
            
            current_sentence_embedding = sentence_embeddings[i].reshape(1, -1)
            similarity = cosine_similarity(prev_chunk_embedding, current_sentence_embedding)[0][0]
            ...
            if similarity >= self.similarity_threshold:
                ...
                chunk_end = text.find(sentences[i-1]) + len(sentences[i-1])
                chunks.append((chunk_content, current_chunk_start, chunk_end))
                
                # Start new chunk
                current_chunk_sentences = [sentences[i]]
                current_chunk_start = text.find(sentences[i])
        
        # Add the last chunk
        if current_chunk_sentences:
            chunk_content = ' '.join(current_chunk_sentences)
            # Find the end position of the last sentence
            last_sentence = current_chunk_sentences[-1]
            chunk_end = text.find(last_sentence) + len(last_sentence)
            chunks.append((chunk_content, current_chunk_start, chunk_end))
...
def main():
    """Run the standalone chunking evaluation."""
    print("Running standalone chunking evaluation...")
    ...
    import json
    with open("standalone_chunking_evaluation_results.json", "w") as f:
        json.dump(chunking_results, f, indent=2)
    
    print("\nDetailed results saved to standalone_chunking_evaluation_results.json")
